...
const TaskModal: React.FC<TaskModalProps> = ({ isOpen, task, onClose, onSave, onDelete }) => {
  ...
  const handleSave = useCallback(() => {
    if (task) {
      onSave({ ...task, title: editableTitle.trim() || "Untitled Task", description: editableDescription });
    }
    ...
    <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center p-4 z-50 transition-opacity duration-300"
      ...
      <div className="bg-gray-800 p-6 rounded-lg shadow-xl w-full max-w-lg text-gray-100 transform transition-all duration-300 scale-100 opacity-100">
        <div className="flex justify-between items-center mb-4">
          <h2 id="taskModalTitle" className="text-2xl font-semibold text-sky-400">Edit Task</h2>
          <button
            onClick={onClose}
        ...
        <div className="mb-4">
          <label htmlFor="taskTitle" className="block text-sm font-medium text-gray-300 mb-1">Title</label>
          <input
            id="taskTitle"
            ref={titleInputRef}
            type="text"
