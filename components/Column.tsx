...
interface ColumnProps {
  ...
  onTaskDragStart: (event: React.DragEvent, taskId: string, sourceColumnId: ColumnId) => void;
...
const ColumnComponent: React.FC<ColumnProps> = ({
  ...
  // Tasks are pre-sorted by App.tsx before being passed if needed, 
  // but an explicit sort here ensures they are always ordered by taskOrder for display.
    ...
    >
      <div className="flex justify-between items-center sticky top-0 bg-gray-800 py-3 z-10 -mx-4 px-4 border-b border-gray-700 mb-3">
        <h2 className="text-xl font-semibold text-sky-300">
          {column.title} <span className="text-sm font-normal text-gray-400">({sortedTasks.length})</span>
        </h2>
        <button
